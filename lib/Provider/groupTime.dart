...
class GroupTime {
  ...
  Future<int> addGroupSchedule(String groupId, Duration dur, bool down,
      DateTime start, DateTime end, List<dynamic> members, String scheduleName) async {
...
=======
    ...
    for (final schedule in allGroup) {
      ...
      for (final avatime in available) {
        ...
        else if (schedule.from.isBefore(avatime.startTime) || schedule.to.isBefore(avatime.endTime)) {
        ...
        else if (schedule.from.isAfter(avatime.startTime) || schedule.to.isAfter(avatime.endTime)) {
    ...
    print('all available time for meeting after filtering: ${available.length}');
